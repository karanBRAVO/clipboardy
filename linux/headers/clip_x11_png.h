...
bool write_png(const image& image, std::vector<uint8_t>& output) {
  ...
  int color_type = (spec.alpha_mask ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB);

  png_set_IHDR(png, info, spec.width, spec.height, 8, color_type, PNG_INTERLACE_NONE,
  ...
  for (png_uint_32 y = 0; y < spec.height; ++y) {
    const uint32_t* src =
        (const uint32_t*)(((const uint8_t*)image.data()) + y * spec.bytes_per_row);
    uint8_t* dst = row;
    unsigned int x, c;
...
bool read_png(const uint8_t* buf, const size_t len, image* output_image, image_spec* output_spec) {
  ...
  png_get_IHDR(png, info, &width, &height, &bit_depth, &color_type, &interlace_type, nullptr,
               nullptr);
  ...
  if (output_spec) *output_spec = spec;
  ...
  if (output_image && width > 0 && height > 0) {
    ...
    png_uint_32 y;
    for (y = 0; y < height; ++y) rows[y] = (png_bytep)png_malloc(png, src_bytes_per_row);

    for (int pass = 0; pass < number_passes; ++pass)
      for (y = 0; y < height; ++y) png_read_rows(png, rows + y, nullptr, 1);
