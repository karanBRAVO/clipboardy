...
class Manager {
 public:
      ...
      if (x11_clipboard_manager) {
        ...
        if (try_lock()) {
          ...
          get_data_from_selection_owner(
              {get_atom(SAVE_TARGETS)}, []() -> bool { return true; }, x11_clipboard_manager);
        }
      }
    }
#endif
  ...
  void clear() {
    ...
    // could ignore).
    if (set_x11_selection_owner()) return;
  ...
  bool is_convertible(format f) const {
    ...
    // Ask to the selection owner the available formats/atoms/targets.
      ...
      return get_data_from_selection_owner({get_atom(TARGETS)}, [this, &atoms]() -> bool {
        assert(m_reply_data);
        if (!m_reply_data) return false;
  ...
  bool set_data(format f, const char* buf, size_t len) {
    if (!set_x11_selection_owner()) return false;
    ...
    for (xcb_atom_t atom : atoms) m_data[atom] = shared_data_buf;
  ...
  bool get_data(format f, char* buf, size_t len) const {
    ...
    if (owner == m_window) {
      ...
      if (get_data_from_selection_owner(atoms, [this, buf, len, f]() -> bool {
            size_t n = std::min(len, m_reply_data->size());
            std::copy(m_reply_data->begin(), m_reply_data->begin() + n, buf);

            if (f == text_format()) {
              if (n < len) buf[n] = 0;  // Include a null character
            }
      ...
      if (m_image.is_valid()) {
        ...
        return true;
                          ...
                          {get_atom(MIME_IMAGE_PNG)}, [this, &output_img]() -> bool {
                            return x11::read_png(&(*m_reply_data)[0], m_reply_data->size(),
                                                 &output_img, nullptr);
                          })) {
      return true;
    }
    ...
    if (owner == m_window) {
      if (m_image.is_valid()) {
        ...
        return true;
             ...
             get_data_from_selection_owner({get_atom(MIME_IMAGE_PNG)}, [this, &spec]() -> bool {
 ...
 private:
  ...
  void handle_selection_request_event(xcb_selection_request_event_t* event) {
    ...
    if (event->target == get_atom(TARGETS)) {
...
#endif
      for (const auto& it : m_data) targets.push_back(it.first);

      // Set the "property" of "requestor" with the clipboard
      // formats ("targets", atoms) that we provide.
      xcb_change_property(m_connection, XCB_PROP_MODE_REPLACE, event->requestor, event->property,
                          get_atom(ATOM), 8 * sizeof(xcb_atom_t), targets.size(), &targets[0]);
    }
#ifdef CLIP_SUPPORT_SAVE_TARGETS
      ...
      if (reply) {
        for (xcb_atom_t *ptr = (xcb_atom_t*)xcb_get_property_value(reply),
          ...
          if (!set_requestor_property_with_clipboard_content(event->requestor, property, target)) {
            xcb_change_property(m_connection, XCB_PROP_MODE_REPLACE, event->requestor,
                                event->property, XCB_ATOM_NONE, 0, 0, nullptr);
          }
        }
      ...
      if (!set_requestor_property_with_clipboard_content(event->requestor, event->property,
                                                         event->target)) {
  ...
  bool set_requestor_property_with_clipboard_content(const xcb_atom_t requestor,
    ...
    xcb_change_property(m_connection, XCB_PROP_MODE_REPLACE, requestor, property, target, 8,
  ...
  void handle_selection_notify_event(xcb_selection_notify_event_t* event) {
    ...
    xcb_get_property_reply_t* reply =
        get_and_delete_property(event->requestor, event->property, m_target_atom);
    if (reply) {
  ...
  void handle_property_notify_event(xcb_property_notify_event_t* event) {
    if (m_incr_process && event->state == XCB_PROPERTY_NEW_VALUE &&
      ...
      xcb_get_property_reply_t* reply =
          get_and_delete_property(event->window, event->atom, m_target_atom);
      if (reply) {
        m_incr_received = true;
  ...
  xcb_get_property_reply_t* get_and_delete_property(xcb_window_t window, xcb_atom_t property,
                                                    xcb_atom_t atom, bool delete_prop = true) {
    xcb_get_property_cookie_t cookie =
        xcb_get_property(m_connection, delete_prop, window, property, atom, 0,
  ...
  void call_callback(xcb_get_property_reply_t* reply) {
    ...
    if (m_callback) m_callback_result = m_callback();
  ...
  bool get_data_from_selection_owner(const atoms& atoms, const notify_callback&& callback,
    ...
    if (!selection) selection = get_atom(CLIPBOARD);
    ...
    if (m_window != get_x11_selection_owner()) m_data.clear();
    ...
    for (xcb_atom_t atom : atoms) {
      ...
      do {
        ...
        std::cv_status status =
            m_cv.wait_for(m_lock, std::chrono::milliseconds(get_x11_wait_timeout()));
  ...
  xcb_atom_t get_atom(CommonAtom i) const {
    if (m_common_atoms.empty()) {
      m_common_atoms =
          get_atoms(kCommonAtomNames, sizeof(kCommonAtomNames) / sizeof(kCommonAtomNames[0]));
    }
  ...
  std::string get_atom_name(xcb_atom_t atom) const {
    ...
    if (reply) {
      ...
      if (len > 0) {
        ...
        if (name) std::copy(name, name + len, result.begin());
      }
      free(reply);
    }
    return result;
