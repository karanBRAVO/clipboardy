...
class Hglobal {
 public:
  ...
  ~Hglobal() {
    if (m_handle) GlobalFree(m_handle);
  }
...
class AnonymousTokenImpersonator {
 public:
  AnonymousTokenImpersonator() : m_must_revert(ImpersonateAnonymousToken(GetCurrentThread())) {}

  ~AnonymousTokenImpersonator() {
    if (m_must_revert) RevertToSelf();
  }
...
bool lock::impl::get_data(format f, char* buf, size_t len) const {
  ...
  if (!buf || !is_convertible(f)) return false;
  ...
  if (f == text_format()) {
    if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
      ...
      if (hglobal) {
        LPWSTR lpstr = static_cast<LPWSTR>(GlobalLock(hglobal));
        if (lpstr) {
          size_t reqsize = WideCharToMultiByte(CP_UTF8, 0, lpstr, -1, nullptr, 0, nullptr, nullptr);
    ...
    if (IsClipboardFormatAvailable(f)) {
      ...
      if (hglobal) {
        ...
        if (ptr) {
          ...
          if (reqsize > total_size) reqsize = total_size - sizeof(CustomSizeT);
  ...
  if (f == text_format()) {
    if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
      ...
      if (hglobal) {
        LPWSTR lpstr = static_cast<LPWSTR>(GlobalLock(hglobal));
        if (lpstr) {
          len = WideCharToMultiByte(CP_UTF8, 0, lpstr, -1, nullptr, 0, nullptr, nullptr);
          GlobalUnlock(hglobal);
        }
      }
    } else if (IsClipboardFormatAvailable(CF_TEXT)) {
    ...
    if (IsClipboardFormatAvailable(f)) {
      ...
      if (hglobal) {
        ...
        if (ptr) {
          ...
          if (len > total_size) len = total_size - sizeof(CustomSizeT);
...
std::vector<format_info> lock::impl::list_formats() const {
  static const char* standard_formats[CF_MAX] = {
      ...
      "CF_TIFF", "CF_OEMTEXT",     "CF_DIB",         "CF_PALETTE",      "CF_PENDATA", "CF_RIFF",
      "CF_WAVE", "CF_UNICODETEXT", "CF_ENHMETAFILE", "CF_HDROP",        "CF_LOCALE",  "CF_DIBV5"};
  ...
  while (format_id != 0) {
    ...
    // Get user-defined format name
      ...
      int size = GetClipboardFormatNameA(format_id, format_name.data(), format_name.size());
...
bool lock::impl::set_image(const image& image) {
  ...
  if (spec.bits_per_pixel == 32 && spec.alpha_mask) {
    ...
    if (png_format) {
      ...
      if (png_handle) SetClipboardData(png_format, png_handle);
    }
  }
